...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap:
            new_xdomains = copy.deepcopy(self.domains[x])
            for words_x in new_xdomains:
                flag = 0
                for words_y in self.domains[y]:
                    if words_x[overlap[0]] == words_y[overlap[1]]:
                        flag = 1
                if flag == 0:
                    revised = True
                    self.domains[x].remove(words_x)
        return revised
        ...
        while len(arcs) > 0:
            ...
            if self.revise(X, Y):
                if len(self.domains[X]) == 0:
                    return False
                for val in self.crossword.neighbors(X):
                    if val != Y:
                        arcs.append((val, X))
        return True
